...
namespace Covid19Functions
{
    public static class AnalyzeReportFunction
    {
            ...
            ILogger log)
        {
            ...
            {
                analysis = new ReportAnalyzed(report.UserName, report.Email, report.Position, true, "Appelez 15");
            }
            else if (Enum.GetValues(typeof(Symptoms)).Cast<Symptoms>().Count(x => report.Symptoms.HasFlag(x)) > 1)
            {
                analysis = new ReportAnalyzed(report.UserName, report.Email, report.Position, true, "Appeler votre médecin.");
            }
            else if (report.Symptoms != Symptoms.None)
            {
                analysis = new ReportAnalyzed(report.UserName, report.Email, report.Position, false, "Continuez à surveiller votre état et restez chez-vous.");
            }
            else
            {
                analysis = new ReportAnalyzed(report.UserName, report.Email, report.Position, false, "Vous êtes très bien ne vous inquiétez pas.");
            }
